        ...
        if len(guess) == len(wordle):
            for i in range(len(guess)):
                if guess[i] == wordle[i]:
                    ...
                    if guess[i] in wordle_occurrences:
                        if wordle_occurrences[guess[i]] < wordle.count(guess[i]):
                            wrong_spot += 1
                    else:
                        wrong_spot += 1
                    wordle_occurrences[guess[i]] = wordle_occurrences.get(guess[i], 0) + 1
...
print(f"Wordles solved: {wordles_solved}")
print(f"Wordles unsolved: {wordles_unsolved}")
